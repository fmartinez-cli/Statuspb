...
$empezar_desde = ($pagina-1) * $cantidad_resultados_por_pagina;
			...
			$consulta=mysqli_query($enlace, "SELECT DISTINCT(WO) from racksterminados WHERE Modelo='Microsoft 8.2' ORDER BY ID DESC");
			$total_registros = mysqli_num_rows($consulta);

			$consulta_resultados=mysqli_query($enlace, "SELECT DISTINCT(WO) from racksterminados WHERE Modelo='Microsoft 8.2' ORDER BY ID DESC LIMIT $empezar_desde, $cantidad_resultados_por_pagina" );
			echo '<br><h2 style="text-align:center;">Microsoft 8.2</h2><br><br>';
			...
			while($datos=mysqli_fetch_row($consulta_resultados)){
						...
						if(!$datos2=mysqli_fetch_row($consulta2)){
							 ...
							 <button style=" width:400px" class="btn1">WO: '.$datos[0].'</button></form>';


						}

								// echo '<h2 style="float:right;">Dropbox</h2><br><br><form action="buscar.php?WO='.$datos[0].'" method="post" >
							 // <input type="hidden" id="WO" name="WO" value="'.$datos[0].'">
							 // <button style="float:right; background-color:#049372;" class="btn1">'.$datos[0].' en piso</button></form>';
...
echo "<a class='enlace' href='estadisticas.php'>1</a> | ";
for ($i=2; $i<=$total_paginas; $i++) {
	//En el bucle, muestra la paginación
	echo "<a class='enlace' href='?pagina=".$i."'>".$i."</a> | ";
};
echo ' <a style="color:#049372; float:right" href="ordenesdrop.php" class="enlace">Dropbox</a>'
...
</br><button name="buscarbtn" form="buscar" style="width:400px;" class="btn1" id="buscarbtn"
