...
if ($update) {
   ...
   echo "<span style='color:#CF000F; font-size:20px;'> Hubo un error al actualizar</span>";
	...
	if($cons['Rackscan']=='1'&&$cons['Solar']==0){
...
if ($update) {
   ...
   echo "<span style='color:#CF000F; font-size:20px;'> Hubo un error al actualizar</span>";
	...
	//_______________________________________________________________________
	if($cons['Solar']=='1'&&$cons['Cluster']==0){
...
if ($update) {
   ...
   echo "<span style='color:#CF000F; font-size:20px;'> Hubo un error al actualizar</span>";
	...
	//_______________________________________________________________________
	if($cons['Cluster']=='1'&&$cons['Solar2']==0){
...
if ($update) {
   ...
   echo "<span style='color:#CF000F; font-size:20px;'> Hubo un error al actualizar</span>";
	...
	//_______________________________________________________________________
	if($cons['Solar2']=='1'&&$cons['Sharknado']==0){
		$update=$enlace->query("UPDATE pruebas set  Solar2='0', Solar2NoReloj=NULL, Solar2HoraFinal=NULL where NoSerie = '$NoSerie'");
if ($update) {
    echo "<script>location.href = 'retrocederrack.php'</script>";
} else {
   echo "<span style='color:#CF000F; font-size:20px;'> Hubo un error al actualizar</span>";
	...
	//_______________________________________________________________________
		if($cons['Sharknado']=='1'&&$cons['Wiring']==0){
