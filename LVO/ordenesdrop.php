...
<html>
<head>
	<title>Ordenes terminadas Dropbox</title>
...
<body class="desarroll">
	...
	</header><?php if(isset($_SESSION['Nombre'])){
				echo '<section>
...
$empezar_desde = ($pagina-1) * $cantidad_resultados_por_pagina;
			...
			$consulta=mysqli_query($enlace, "SELECT DISTINCT(WO) from racksterminados  WHERE Modelo='Dropbox' ORDER BY ID DESC");
			$total_registros = mysqli_num_rows($consulta);

			$consulta_resultados=mysqli_query($enlace, "SELECT DISTINCT(WO) from racksterminados WHERE Modelo='Dropbox' ORDER BY ID DESC LIMIT $empezar_desde, $cantidad_resultados_por_pagina" );
			echo '<br><center><h2 style="float:none;">Dropbox</h2></center><br><br>';
			$total_paginas = ceil($total_registros / $cantidad_resultados_por_pagina);
			while($datos=mysqli_fetch_row($consulta_resultados)){
						$consulta2=mysqli_query($enlace, "SELECT * from racks WHERE Wo='$datos[0]'");

						echo '<center><div >';
						if(!$datos2=mysqli_fetch_row($consulta2)){

						echo '<form action="buscar.php?WO='.$datos[0].'" method="post" >
							 <input type="hidden" id="WO" name="WO" value="'.$datos[0].'">
							 <button style=" width:400px; background-color:#049372;" class="btn1">WO: '.$datos[0].'</button></form>';
...
echo "<a style='color:#049372;' class='enlace' href='ordenesdrop.php'>1</a> | ";
for ($i=2; $i<=$total_paginas; $i++) {
	//En el bucle, muestra la paginación
	echo "<a style='color:#049372;' class='enlace' href='?pagina=".$i."'>".$i."</a> | ";
};

	echo ' <a style="float:right; color:#2574a9" href="estadisticas.php" class="enlace">Azure 4.1</a>'
        ...
        <br><br>
				<center><div>
<form id="buscar" name="buscar" method="POST" action="return false" onsubmit="return false" >
<input type="text" title="WO - Ej: 123123123" name="WO2" form="buscar" class="inputl" id="WO2"
required placeholder="WO" pattern="[0-9]{9}" maxlength="9">
</br><button name="buscarbtn" form="buscar" style="width:400px; background:#049372;" class="btn1" id="buscarbtn"
onclick="Validar(document.getElementById('WO2').value);">Buscar</button>
</form>
</div>
<div id="resultado" style="font-size:22px;">&nbsp;</div></center>
			</div>
		</div>
