...
include('css/themes.php');
  ...
  <script src="js/echarts_wo_min.js"></script>
...
<form class="buscador" action="" method="post">
    ...
    <button class="btn2" style="width:300px">Buscar</button>
		...
		if (isset($_POST['WO'])) {
			...
			$consulta = mysqli_query($enlace, "SELECT DISTINCT(WO) FROM racks_copy WHERE (Modelo='Microsoft 8.2' OR Modelo='Microsoft 8.3' OR Modelo='NPI') AND WO='$WO'");
			while ($datos = mysqli_fetch_row($consulta)) {
				...
				while ($datoswo = mysqli_fetch_row($consultawo)) {
					...
					while ($datoswop = mysqli_fetch_row($consultawop)) {
						...
						echo '<td class="tdinfo2" contenteditable="false" id="comentarios_' . $datoswo[0] . '">' . $datoswo[8] . '</td></tr>';
					}
				}
				echo '</table>';
				echo '<br><br>';
...
// AQUI VA LA LOGICA PARA EL GRAFICO DE BARRAS DE LA WO
