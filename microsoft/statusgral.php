...
// FUNCIÓN PARA TIEMPO PROGRESIVO
function formatearTiempoProgresivo($horaInicio, $horaFinal) {
    if (empty($horaInicio) || $horaInicio == 'Vacio' || $horaInicio == null) {
        return 'No iniciado';
    }
    
    if (empty($horaFinal) || $horaFinal == 'Vacio' || $horaFinal == null) {
    ...
    try {
        ...
        if ($interval->days > 0) {
            return $interval->format('%d días, %h h, %i min');
        } elseif ($interval->h > 0) {
...
// Verificar si el usuario está logueado Y si su nivel es 1 o 99 (admin)
if (!isset($_SESSION['Nombre']) || ($_SESSION['Nivel'] != 1 && $_SESSION['Nivel'] != 99)) {
    ?>
    <center>
        <br><br><br>
...
<!------------------------------------------------------- LEYENDA DE COLORES ------------------------------------------------->
<div style="margin: 15px 0; padding: 10px; background-color: #f0f0f0; border-radius: 5px; border: 1px solid #ccc; display: inline-block;">
    <strong style="margin-right: 15px;">Estados:</strong>
    <span style="margin-right: 15px;"><span style="display: inline-block; width: 20px; height: 12px; background-color: #008000; border: 1px solid #666;"></span> Pass</span>
    <span style="margin-right: 15px;"><span style="display: inline-block; width: 20px; height: 12px; background-color: #f7a307; border: 1px solid #666;"></span> Waiting</span>
    <span style="margin-right: 15px;"><span style="display: inline-block; width: 20px; height: 12px; background-color: #FFFF00; border: 1px solid #666;"></span> Running</span>
    <span style="margin-right: 15px;"><span style="display: inline-block; width: 20px; height: 12px; background-color: #FF0000; border: 1px solid #666;"></span> Fail</span>
</div>
<!------------------------------------------------------- FIN LEYENDA DE COLORES ------------------------------------------------->

<!------------------------------------------------------- Tabla de la pagina web ------------------------------------------------->
			<?php
$consulta = mysqli_query($enlace, "SELECT DISTINCT(WO) FROM racksmicro WHERE Modelo='Microsoft 8.2' OR Modelo='Microsoft 8.3' OR Modelo='NPI'");
while ($datos = mysqli_fetch_row($consulta)) {
    ...
    echo '<table class="stgral"><caption><h2>WO: ' . $datos[0] . '</h2></caption>
        ...
        ';
					...
					$consultawo = mysqli_query($enlace, "SELECT * from racksmicro where WO='$datos[0]'");
    ...
    switch ($modelo) {
        case 'Microsoft 8.2':
            $colorModelo = '#2574A9'; // Azul
            break;
        case 'Microsoft 8.3':
            $colorModelo = '#049372'; // Verde
            break;
        case 'NPI':
            $colorModelo = '#e26715'; // Naranja
            break;
        default:
            $colorModelo = '#3e3e3e'; // Gris oscuro para otros
            break;
    }
    
    echo "<td class='tdinfo2' style='background-color: $colorModelo; color: white; font-weight: bold;'>" . $modelo . "</td>";

    // Consulta a pruebasmicro con fetch_assoc() para usar nombres de columna
    $consultawop = mysqli_query($enlace, "SELECT * FROM pruebasmicro WHERE NoSerie='$datoswo[0]'");
    $contador++;

    if ($consultawop && mysqli_num_rows($consultawop) > 0) {
        $datoswop = mysqli_fetch_assoc($consultawop);
        
        // Definir las columnas de pruebas en orden
        $pruebas_columnas = ['FTO', 'QuickTest', 'Stress', 'MDaaS', 'RackTest', 'FTI', 'BSL', 'CTS', 'Packing'];
        
        // Mostrar celdas de cada prueba
        foreach ($pruebas_columnas as $prueba) {
            $statusValue = $datoswop[$prueba] ?? 0;
            $status2Value = $datoswop[$prueba . 'Status2'] ?? '';
            $cellColor = '';
            
            if ($statusValue == 1) {
                $cellColor = '#008000'; // Verde para completado
                ...
                // Si es 'Vacio' o vacío, no se asigna color (celda vacía)
            }
            
            echo "<td bgcolor='$cellColor'></td>";
        }

        // Validar horas para calcular tiempo transcurrido (FORMATO PROGRESIVO)
        $horaInicio = $datoswop['HoraInicio'] ?? '';
        $horaFinal = $datoswop['PackingHoraFinal'] ?? '';

        $tiempoTranscurrido = formatearTiempoProgresivo($horaInicio, $horaFinal);

        echo '<td class="tdinfo2">' . ($datoswop['HoraInicio'] ?? '') . '</td>';
        echo '<td class="tdinfo2">' . ($datoswop['PackingHoraFinal'] ?? '') . '</td>';
        echo '<td class="tdinfo2">' . $tiempoTranscurrido . '</td>';
        echo '<td class="tdinfo2" contenteditable="true" id="comentarios_' . $datoswo[0] . '">' . ($datoswo[8] ?? '') . '</td></tr>';
    } else {
        // Si no hay datos en pruebasmicro, mostrar celdas vacías
        for ($i = 0; $i < 9; $i++) {
            echo "<td bgcolor=''></td>";
        }
        echo '<td class="tdinfo2"></td><td class="tdinfo2"></td><td class="tdinfo2">Sin datos</td>';
        echo '<td class="tdinfo2" contenteditable="true" id="comentarios_' . $datoswo[0] . '">' . ($datoswo[8] ?? '') . '</td></tr>';
...
<center><button id="crearimagen" class="form-control btn1">Crear Imagen</button></center>
	...
	<div class="grupo">
	<center><div id="img-out" class="cimagen" style="text-align:center; "></div></center>
</div>
<script type="text/javascript">
	$(function() {
