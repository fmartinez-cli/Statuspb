...
<script>
	function Validar(NoReloj)
	{
		$.ajax({
			url: "borrartecnicobackend.php",
			type: "POST",
			data: "NoReloj="+NoReloj,
...
<center>
					<div class="grupo">
						<div class="caja">
							<?php
							$Bahia=$_SESSION['Bahia'];
							$Turno=$_SESSION['Turno'];
							$query= mysqli_query($enlace, "SELECT * FROM users where Bahia='$Bahia' AND Nivel='2' AND Turno='$Turno'") ;
							echo '<div class="editartec" style="color:#CC263A;"><h2>Eliminar t&eacute;cnico</h2>';
							while ($datos=mysqli_fetch_row($query) ) {
								echo '
								<div><form action="borrartecnicoform.php" method="post" >
								<input type="hidden" name="NoReloj" value="'.$datos["0"].'">
								<button class="btn1" style="width:800px;"><b>Nombre: <b>'.$datos["2"].'<b>  &nbsp;&nbsp;&nbsp;  <b>No. Reloj:</b> '.$datos["0"].' &nbsp;&nbsp;&nbsp;  <b>Turno:</b> '.$datos["3"].'  &nbsp;&nbsp;&nbsp;  <b>Bahia:</b> '.$datos["5"].'</button>
								</form>
								</div>
								...
								echo '</div>';
								?>
								<form name="buscartec" action="return false" onsubmit="return false" method="post">
									<input class="inputl" required type="text" name="NoReloj" id="NoReloj" placeholder="N&uacute;mero de reloj" maxlength="5" pattern="[0-9]{5}">
									<br><br><br><button class="btn1" onclick="Validar(document.getElementById('NoReloj').value);">Buscar</button>
								</form>
								<br><div id="resultado" style="font-size:22px;">&nbsp;</div>
								<td><br><button class="btn1" form="none" onclick="location.href = 'administrar.php'">volver</button>
        </div>
      </div>
  </center>
							</section>
