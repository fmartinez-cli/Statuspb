 <?php 
session_start();
if($_SESSION['Nivel']!='1'||!isset($_REQUEST['Nombre'])){
...
if(!preg_match("/^[0-9]{5}$/", $NoReloj)){
		...
		echo "<span style='color:white; background:#CF000F; margin-top:480px; padding:10px; width:500px; '> Seleccione una bahia</span>";
			...
			$update=$enlace->query("UPDATE users set  No_Reloj='$NoReloj', Nombre='$Nombre', Pass='$Passf', Turno='$Turno', Bahia='$Bahia' where No_Reloj = '$oldnoreloj'");
if ($update) {

    echo "<script>location.href = 'editartecnico.php'</script>";
} else {
   echo "<span style='color:white; background:#CF000F; margin-top:480px; padding:10px; width:500px; '> El n&uacute;mero de reloj ya se encuentre ragistrado</span>";
}
