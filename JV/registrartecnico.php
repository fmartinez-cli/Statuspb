...
if (!isset($_SESSION['Nombre'])){
	...
	if($_SESSION['Nivel']!='1'){
		...
		<html>
		<head>
			<title>Registrar t&eacute;cnico</title>
...
<script>
	function Validar(NoReloj, Nombre, Pass, Pass2, Turno, Bahia)
	{
		$.ajax({
			url: "registrartecnicobackend.php",
			type: "POST",
			data: "NoReloj="+NoReloj+"&Nombre="+Nombre+"&Pass="+Pass+"&Pass2="+Pass2+"&Turno="+Turno+"&Bahia="+Bahia,
...
<table class="formsadminjv">
<tr><th><h2 style="color:#CC263A;">Agregar t&eacute;cnico</h2></th></tr>
<tr><td><br><input type="text" title="Ejemplo: N&uacute;mero de reloj. Ej: 12345" name="NoReloj" form="registrotec" class="inputl" id="NoReloj" required placeholder="N&uacute;mero de reloj" pattern="[0-9]{5}" maxlength="5"></td></tr>
<tr><td><input type="text" title="Nombre" style="text-transform:none;" name="Nombre" form="registrotec" class="inputl" id="Nombre" required placeholder="Nombre" pattern="[a-zA-Z/\s/ñáéíóúÑ]{1,}" maxlength="30"></td></tr>
<tr><td><input type="password" name="Pass" form="registrotec" class="inputl" id="Pass" required placeholder="Contrase&ntilde;a"  maxlength="15"></td></tr>
<tr><td><input type="password" name="Pass2" form="registrotec" class="inputl" id="Pass2" required placeholder="Confirmar contrase&ntilde;a"  maxlength="15"></td></tr>
...
<option value="" disabled selected>Bahia</option>
<option value="1">1</option>
<option value="2">2</option>
<option value="3">3</option>
...
document.getElementById('Pass2').value, document.getElementById('Turno').value, document.getElementById('Bahia').value);">Registrar</button></td></tr>
<tr><td><button class="btn1" form="none" onclick="location.href = 'administrar.php'">volver</button></td></tr>
</table>
<div id="resultado" style="font-size:22px;">&nbsp;</div>	<br>
